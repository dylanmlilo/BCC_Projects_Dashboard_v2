...
class Output(Base):
    ...
    def gis_output_data_to_dict_list(cls):
        """
        Queries the database for Output data and converts it to a list of dictionaries.
...
class Activity(Base):
  ...
  def gis_activity_data_to_dict_list(cls):
    """
    Queries the database for Activity data and converts it to a list of dictionaries.
    Returns:
        list: A list of dictionaries containing activity data.
    """
    activity_list = []
    try:
        query = session.query(cls.id, cls.activity, cls.output_id, cls.responsible_person_id).all()
    except:
        session.rollback()
    finally:
        session.close()
